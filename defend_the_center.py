...
# THESE WON'T BE USED. SEE MAIN FUNCTION.
...
# class ReplayMemory:
#     def __init__(self, capacity, kframes):
#         self.kframes = kframes
#         state_shape = (capacity, resolution[0], resolution[1])
#         self.s1 = np.zeros(state_shape, dtype=np.float32)
#         self.s2 = np.zeros(state_shape, dtype=np.float32)
...
# def create_model(available_actions_count, kframes):
...
def create_model(available_actions_count, kframes):
    ...
    conv1 = Conv2D(8, 6, strides=1, activation='relu', data_format="channels_first")(state_input)  # filters, kernal_size, stride
    conv2 = Conv2D(8, 3, strides=1, activation='relu', data_format="channels_first")(conv1)  # filters, kernal_size, stride
    flatten = Flatten()(conv2)
    fc1 = Dense(128, input_shape=(192,), activation='relu')(flatten)
    fc2 = Dense(available_actions_count, input_shape=(128,))(fc1)
...
if __name__ == '__main__':
    ...
    memory = ReplayMemory(replay_memory_size, kframes, resolution, 10000)

    if load_model and os.path.isfile(model_savefile):
        print("Loading model from: ", model_savefile)
        model = lm(model_savefile)
    else:
        my_input, model = create_model(len(actions), kframes)
