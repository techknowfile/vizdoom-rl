...
class ReplayMemory:
    def __init__(self, capacity):
        channels = 1
        memory_shape = (capacity, channels, resolution[0], resolution[1] // kframes)
        self.s1 = np.zeros(memory_shape, dtype=np.float32)
        self.s2 = np.zeros(memory_shape, dtype=np.float32)
...
def learn_from_memory(model, memory):
...
def perform_learning_step(epoch, memory):
    ...
    learn_from_memory(model, memory)
...
def main():
    ...
    parser.add_argument('-s', '--scenario', help='Doom scenario to use')
    parser.add_argument('-w', '--save_model', type=bool, default=None)
    parser.add_argument('-l', '--load_model', help='Load existing model. Used to train or run existing model, or run an existing model if skip_learning enabled')
    parser.add_argument('-r', '--image_resolution', nargs='+', type=int)
    ...
    # Set resolution of state (stretch width of frames by kframes)
    config['resolution'] = [config['image_resolution'][0], config['image_resolution'][1]*config['kframes']]

    # Set shape of frame (to match CNN requirements)
    # TODO: Get rid of constants for channel value
    config['frame_shape'] = [1, 1, config['image_resolution'][0], config['image_resolution'][1]]

    if not os.path.exists('models'):
        os.makedirs('models')

    game = initialize_vizdoom(config['scenario'])
    ...
    if config['load_model']:
        ...
        model = lm(config['model_savefile'])
        pass
    else:
    ...
    if not config['skip_learning']:
        for epoch in range(config['epochs']):
            ...
            print("Training...")
            game.new_episode()
            for learning_step in trange(config['learning_steps_per_epoch'], leave=True):
                perform_learning_step(epoch, memory)
    ...
    for _ in range(config['episodes_to_watch']):
        game.new_episode()
        sb = StateBuilder((1, 1, config['resolution'][0], config['resolution'][1] // config['kframes']), frames_per_state=config['kframes'])
        while not game.is_episode_finished():
            frame = preprocess(game.get_state().screen_buffer)
            frame = frame.reshape([1, 1, resolution[0], resolution[1] // kframes])
            state = sb.get_state(frame)
